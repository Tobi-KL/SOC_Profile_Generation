...
def rank_households(number_of_occupants,
                    number_of_drivers,
                    number_of_cars,
                    income,
                    w_income,
    ...
    for i in range(0, len(meta_mop_data)): 
        household = Household(meta_mop_data[i, 0])

        # hh is looked at more closely, only if all meta factors are fulfilled
    ...
    for i in range(0, len(households)):
        household = Household(household_ID = households[i])
    ...
    # sort hosueholds by overall score and return 10 best fitting
    households_fitting = [x for _, x in sorted(zip(score_array, households))]
    households_10_best_fitting = list(reversed(households_fitting)) [0:10]

    return households_10_best_fitting